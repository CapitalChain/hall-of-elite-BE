...
export class PayoutService {
   ...
   */
  async initializePayoutTiers(): Promise<void> {
    for (const tierConfig of DEFAULT_PAYOUT_TIERS) {
      const existing = await prisma.payoutTier.findFirst({
        where: {
      ...
      if (!existing) {
        await prisma.payoutTier.create({
          data: tierConfig,
        });
      }
    }
   ...
   */
  async updateTraderPayout(input: PayoutCalculationInput): Promise<TraderPayoutDTO> {
    const calculation = this.calculatePayoutTier(input);

    const payoutTier = await prisma.payoutTier.findFirst({
      where: {
    ...
    const traderPayout = await prisma.traderPayout.upsert({
   ...
   */
  async getTraderPayout(traderId: string): Promise<TraderPayoutDTO | null> {
    const traderPayout = await prisma.traderPayout.findUnique({
      where: { traderId },
      include: {
        payoutTier: true,
      },
  ...
  async getPayoutTiers() {
    return await prisma.payoutTier.findMany({
      orderBy: { order: "asc" },
    });
  }
   ...
   */
  async getTradeStatsForTrader(
    traderId: string
  ): Promise<{ maxTradesPerDay: number; totalTradingDays: number } | null> {
    try {
      ...
      for (const t of trades) {
        const day = (t.closeTime as Date).toISOString().slice(0, 10);
   ...
   */
  async calculatePayoutFromTrades(traderId: string): Promise<TraderPayoutDTO | null> {
    const stats = await this.getTradeStatsForTrader(traderId);
    if (!stats || stats.totalTradingDays === 0) return null;
   ...
   */
  private mapToDTO(record: any): TraderPayoutDTO {
    return {
      id: record.id,
